...
impl Graphics {
    ...
    pub fn process(&self, material: &Material) -> (ShaderModule, PipelineLayout, Vec<BindGroup>) {
        let source = std::fs::read_to_string(material.shader).expect("reading shader failed");
        let shader = self.gpu.device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("test"),
            source: wgpu::ShaderSource::Wgsl(Cow::Borrowed(&source)),
        });

        let (pipeline_layout, groups) = self.gpu.prepare_groups(&material.bindings, None);
        (shader, pipeline_layout, groups)
    }
